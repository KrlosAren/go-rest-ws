...
func (repo *PostgresRepository) InsertUser(ctx context.Context, user *models.User) error {
	_, error := repo.db.ExecContext(ctx, "INSERT INTO users (id,email, password) VALUES ($1,$2, $3)", user.Id, user.Email, user.Password)
	return error
}
...
func (repo *PostgresRepository) InsertPost(ctx context.Context, post *models.Post) error {
	_, error := repo.db.ExecContext(ctx, "INSERT INTO posts (id,post_content, user_id) VALUES ($1,$2, $3)", post.Id, post.PostContent, post.UserId)
	return error
}
...
func (repo *PostgresRepository) DeletePost(ctx context.Context, PostId string, userId string) error {
	_, err := repo.db.ExecContext(ctx, "DELETE FROM posts WHERE id = $1 AND user_id = $2", PostId, userId)
	return err
}
